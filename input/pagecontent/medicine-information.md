...
1. For *coded* support of a medication, the preferred method is to use a single medication code that captures all information required for identification of the medication in code.coding.
    ...
    - When there is a requirement to explicitly state the type of medicinal coding (e.g. branded product with strength or form), [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) can optionally be used in the resource (i.e. MedicationAdministration, MedicationStatement, MedicationDispense, MedicationRequest, Medication):  
      - generic item form and strength = `code.coding` with [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) using `UPDSF` from the [Medication Type code system](https://hl7.org.au/fhir/5.0.0/CodeSystem-medication-type.html)
      - branded item form and strength = `code.coding` with [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) using `BPDSF` from the [Medication Type code system](https://hl7.org.au/fhir/5.0.0/CodeSystem-medication-type.html)
...
2. For *coded* support of brand name, generic name, manufacturer, item form and strength, where the individual components required for medication identification are separately coded:
   - Coded support for the following can be provided using code.coding with an optional [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) to explicitly declare the type of product identification in the resource (i.e. MedicationAdministration, MedicationStatement, MedicationDispense, MedicationRequest, Medication), for example: 
      - brand name = `code.coding` with [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) using `BPD` from the [Medication Type code system](https://hl7.org.au/fhir/5.0.0/CodeSystem-medication-type.html)
      - generic name = `code.coding` with [Medication Type extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-type.html) using `UPD` from the [Medication Type code system](https://hl7.org.au/fhir/5.0.0/CodeSystem-medication-type.html)
...
3.  For *non-coded* support for brand name, generic name, manufacturer, item form and strength:
    - Non-coded support is provided using the Medication resource
        - brand name = `Medication.extension` [Medication Brand Name extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-brand-name.html)
        - generic name = `Medication.extension` [Medication Generic Name extension](https://hl7.org.au/fhir/5.0.0/StructureDefinition-medication-generic-name.html)
